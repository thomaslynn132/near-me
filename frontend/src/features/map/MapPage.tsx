    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (err) => {
        if (err.code === err.PERMISSION_DENIED) {
          alert(
            "Location access denied. Please enable location permissions in your browser settings to use this app.",
          );
        }
      },
...
export default function MapPage() {
  ...
  const randomizePosition = (baseLat: number, baseLng: number, distanceKm: number) => {
    const radiusKm = distanceKm * 0.5 * Math.random();
    const angle = Math.random() * 2 * Math.PI;
    const radiusDeg = radiusKm / 111;
    const newLat = baseLat + radiusDeg * Math.cos(angle);
    const newLng = baseLng + radiusDeg * Math.sin(angle) / Math.cos(baseLat * (Math.PI / 180));
  ...
  const loadNearbyUsers = async () => {
    ...
    try {
      const res = await userApi.getNearby(userLocation[1], userLocation[0], 0.5);
      const users = res.data.data.map((u: any) => ({
        ...u,
        randomPosition: randomizePosition(userLocation[0], userLocation[1], u.distance || 0.5)
      }));
          ...
          <div className="flex gap-4">
            ...
            <div className="flex-1">
              ...
              <div className="flex flex-wrap gap-1 mt-2">
                {selectedUser.interests?.slice(0, 3).map((int: string) => (
                  <span
                    key={int}
                    className="px-2 py-0.5 bg-surface-light rounded-full text-xs">
                    {int}
                  </span>
                ))}
              </div>
            ...
            <div className="flex flex-col gap-2">
              <Button
                size="icon"
                variant="destructive"
                onClick={() => handleDislike(selectedUser._id)}>
                <X className="w-5 h-5" />
              </Button>
              <Button
                size="icon"
                variant="default"
                onClick={() => handleLike(selectedUser._id)}>
                <Heart className="w-5 h-5" />
              </Button>
            </div>
          </div>
          {selectedUser.bio && (
            <p className="mt-3 text-sm text-slate-300">{selectedUser.bio}</p>
          )}
        </div>
      )}
